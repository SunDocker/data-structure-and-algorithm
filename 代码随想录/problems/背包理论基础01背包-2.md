...
**其实可以发现如果<u>把dp[i - 1]那一层拷贝到dp[i]上</u>，表达式完全可以是：$dp[i][j] = max(dp[i][j], dp[i][j - weight[i]] + value[i]);$**

> 也就是说，直接把不用物品i的情况当作初始化情况；
>
> > 为什么一维的不能是物品？因为物品之间**相互独立**
...
在一维dp数组中，dp[j]表示：**容量为j**的背包，所背的物品价值可以最大为dp[j]。
...
dp数组在推导的时候一定是取价值最大的数，如果题目给的价值都是正整数那么<u>非0下标都初始化为0就可以了</u>。
...
二维dp遍历的时候，背包容量是从小到大，而**一维dp遍历的时候，背包是从大到小**。
...
> 01背包迭代需要的之前的值：==**重量较小**且**不使用当前物品**的最大价值==

**那么问题又来了，为什么二维dp数组历的时候不用倒叙呢？**

因为对于二维dp，`dp[i][j]`都是通过上一层即`dp[i - 1][j]`计算而来，本层的`dp[i][j]`并不会被覆盖！所以用不用倒叙都可以

（如何这里读不懂，大家就要动手试一试了，空想还是不靠谱的，实践出真知！）

**再来看看两个嵌套for循环的顺序，代码中是先遍历物品嵌套遍历背包容量，那可不可以先遍历背包容量嵌套遍历物品呢？**

> 对于**二维数组**来说，<u>重量可以放在外层，但只能正序，绝不能倒序</u>，因为每次求当前值，是需要用到==重量较小且不使用当前物品==的情况的，如果重量放在外层还倒序，就根本使用不了重量较小的情况，完全不符合公式迭代条件，非常荒谬！
>
> 对于一维数组来说，根本就不能把重量放在外层，正序的情况下，没有存放==重量较小且不使用当前物品==，每一次使用重量较小的情况时，都是包含了使用当前物品的可能的；倒序的情况下，还是同样的原因，完全不符合公式迭代条件，非常荒谬！

> 你问我为什么重量放在内层就可以倒序了？因为重量较小的情况已经求过并且保存了。啥时候求的？在**上一层外层物品循环**中求的啊！

不可以！

因为一维dp的写法，**<u>背包容量一定是要倒序遍历</u>**（原因上面已经讲了），如果遍历背包容量放在上一层，那么每个dp[j]就只会放入一个物品，即：背包里只放入了一个物品。
