...
这道题目要求从根节点到叶子的路径，所以需要**<u>前序遍历</u>**，这样才方便**<u>让父节点指向孩子节点</u>**，找到对应的路径。
...
那么在如上代码中，**貌似没有看到回溯的逻辑，其实不然，回溯就隐藏在`traversal(cur->left, path + "->", result);`中的 `path + "->"`。**  每<u>次函数调用完，path依然是没有加上"->" 的</u>，这就是回溯了。
...
**大家应该可以感受出来，如果<u>把 `path + "->"`作为函数参数</u>就是可以的，因为并有没有改变path的数值，执行完递归函数之后，path依然是之前的数值（相当于回溯了）**
