...
* 如果==需要搜索**整颗二叉树**且不用处理递归返回值==，递归函数就不要返回值。（这种情况就是本文下半部分介绍的113.路径总和ii）
* 如果==需要搜索**整颗二叉树**且需要**处理递归返回值**==，递归函数就需要返回值。 （这种情况我们在[236. 二叉树的最近公共祖先](https://programmercarl.com/0236.二叉树的最近公共祖先.html)中介绍）
* 如果==要搜索其中**一条符合条件的路径**==，那么递归一定需要返回值，因为遇到符合条件的路径了就要及时返回。（本题的情况）
