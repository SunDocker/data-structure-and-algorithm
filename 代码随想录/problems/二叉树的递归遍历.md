...
1. **确定递归函数的<u>参数</u>和<u>返回值</u>：**
确定哪些参数是递归的过程中需要**处理**的，那么就在递归函数里加上这个参数， 并且还要明确**每次递归的返回值**是什么进而确定递归函数的返回类型。
...
2. **确定终止条件**：在递归的过程中，如何算是递归结束了呢，当然是当<u>前遍历的节点是空</u>了，那么本层递归就要要结束了，所以如果当前遍历的这个节点是空，就直接return，代码如下：
