...
可以发现**滑动窗口的精妙之处在于==根据当前子序列和大小的情况，不断调节子序列的起始位置==。从而将$O(n^2)$的暴力解法降为$O(n)$。**
...
不要以为for里放一个while就以为是$O(n^2)$啊， 主要是看==每一个元素被操作的次数==，每个元素**在滑动窗口进来操作一次，出去操作一次，每个元素都是被被操作两次**，所以时间复杂度是 2 × n 也就是$O(n)$。
